...
/*! \def CDRSTREAMER_MAX_BUFFER_SIZE
    ...
    Now is aprox 4MB
*/
#define CDRSTREAMER_MAX_BUFFER_SIZE	0x3fffff
...
//typedef uint16_t CDRStream_ErrCount_t;
typedef uint32_t CDRStream_ErrCount_t;
...
typedef struct CDRStream_struct_tag{
	...
	CDRStream_ErrCount_t  TotErrs;	//!< indicates the total number of error happened
...
int cdRStreamInit(CDRStream_t* pStream, CDRStream_data_t* pPtrBuff,CDRStream_counters_t pBuffSize);	//!< initialize the streamer data (and structure)
int cdRStreamReset(CDRStream_t* pStream); //!< Restore initial streamer pointer and counter, but not changes buffer size and pointers 

int cdRStreamPutVal(CDRStream_t* pStream, CDRStream_data_t pData); //!< write a byte inside the indicated streamer
int cdRStreamPutArrayVal(CDRStream_t* pStream, CDRStream_data_t *pData, CDRStream_counters_t iNumData);	//!< write a series of vals (uitn16 array) inside the indicated streamer

int cdRStreamRemoveLastVal(CDRStream_t* pStream);	//!<remove last putted byte in stream buffer, and return its value

sint32_t cdRStreamGetNumVals(CDRStream_t* pStream);	//!< returns the number of vals wainting to be read from indicated stream
int cdRStreamPregetVal(CDRStream_t* pStream, CDRStream_data_t* ptrVal );	//!< returns the next available val from the stream, but doesn't remove it from stream, so next time you read the same value
int cdRStreamGetVal(CDRStream_t* pStream, CDRStream_data_t* ptrVal );		//!< returns the next available val from the stream, and remove it from the stream

int cdRStreamGetError(CDRStream_t* pStream);	//!< return the error value (is a bit map value) and reset it in the stream

sint32_t cdRStreamFlush(CDRStream_t* pStream);		//!< empty the stream buffer
sint32_t cdRStreamPutAvilableVals(CDRStream_t* pStream);	//!< return the number of available bytes that could be written into indicated stream

int cdRStreamTocdRStream(CDRStream_t* org, CDRStream_t* dest, CDRStream_counters_t numOfVals); //!< copy numOfVals Vals from org stream to dest stream

sint16_t cdRStreamToUINT8(CDRStream_t* cds); 				//!< extract from stream a unsigned byte 
sint32_t cdRStreamToUINT16(CDRStream_t* cds); 			//!< extract from stream a unsigned 16bits value 
sint32_t cdRStreamToUINT24(CDRStream_t* cds); 				//!< extract from stream a unsigned byte 
int cdRStreamToUINT32(CDRStream_t* cds,uint32_t* ptrVal);  //!< extract from stream a unsigned 32bits value 

int UINT32tocdRStream(CDRStream_t* cds, uint32_t pQuadByte); //!< add four val (formatted like 8bits) to stream by a unsigned 32bits (first val is MSB last is LSB of 32bits)
int UINT24tocdRStream(CDRStream_t* cds, uint32_t pTriByte); //!< add four val (formatted like 8bits) to stream by a unsigned 24bits (low part of 32bits)(first val is MSB last is LSB of 24bits)	
int UINT16tocdRStream(CDRStream_t* cds, uint16_t pDiByte);   //!< add two val (formatted like 8bits) to stream by a unsigned 16bits (first val is MSB last is LSB of 16bits)
int UINT8tocdRStream(CDRStream_t* cds, uint8_t pByte);       //!< add one val (formatted like 8bits) to stream by a unsigned 8bits
